...
class SearchItemTableViewController: UITableViewController, UISearchBarDelegate {
    ...
    let entryUrl = "https://shopping.yahooapis.jp/ShoppingWebService/V3/itemSearch"
    ...
    func createRequestUrl(parameter: [String: String]) -> String {
        ...
        parameter.keys.forEach() {
            guard let value = parameter[$0] else {
                return
            }
            ...
            guard let encodeValue = encodeParameter(key: $0, value: value) else {
                return
            }
    ...
    func request(requestUrl: String) {
        ...
        let task = session.dataTask(with: request) { [weak self]
            ...
            do {
                ...
                self?.itemDataArray.append(contentsOf: resultSet.resultSet)
            } catch let error {
                print("## error: \(error)")
            }
